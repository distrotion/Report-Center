...
router.post('/BP12G_TEST_Report_PDF', async (req, res) => {
  ...
  try {
    if (input['PO'] != undefined) {
      ...
      let getall;
      [getall, find11, DATA] = await Promise.all([
        mongodb.findallC(masterDB),
        mongodb.find(PATTERN, 'GRAPH_TABLE_CONTROL', {}),
    ...
    console.error("BP12G_TEST_Report_PDF error:", error);
...
router.post('/BP12G_TESTAS_Report_by_ref', async (req, res) => {
  ...
  try {
    ...
    console.error("BP12G_TESTAS_Report_by_ref error:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
