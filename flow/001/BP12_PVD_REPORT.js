...
var mongodb = require('../../function/BP12PVDmongodb');
...
router.post('/BP12PVD_Report_PDF', async (req, res) => {
  //-------------------------------------
  console.log("--BP12PVD_Report_PDF--");
  let input = req.body;
  ...
  try {
    ...
    console.error("BP12PVD_Report_PDF error:", error);
...
router.post('/BP12PVD_Report_by_ref', async (req, res) => {
  //-------------------------------------
  console.log("--BP12PVD_Report_by_ref--");
  let input = req.body;
  let DATA = [];
  let DATAlist = [];
  ...
  try {
    ...
    console.error("BP12PVD_Report_by_ref error:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
...
router.post('/BP12PVD_CALDATA', async (req, res) => {
  //-------------------------------------
  console.log("--BP12PVD_CALDATA--");
  let input = req.body;
  let CALDATA = [];
  ...
  try {
    ...
    console.error("BP12PVD_CALDATA error:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
