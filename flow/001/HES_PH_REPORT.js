...
var mongodb = require('../../function/HESPHmongodb');
...
router.post('/HESPH_Report_PDF', async (req, res) => {
  //-------------------------------------
  console.log("--HESPH_Report_PDF--");
  let input = req.body;
  let find1 = [];
  let find2 = [];
  let find3 = [];
  ...
  try {
    ...
    console.error("HESPH_Report_PDF error:", error);
...
router.post('/HESPH_Report_by_ref', async (req, res) => {
  //-------------------------------------
  console.log("--HESPH_Report_by_ref--");
  let input = req.body;
  let DATA = [];
  let DATAlist = [];
  ...
  try {
    ...
    console.error("HESPH_Report_by_ref error:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
...
router.post('/HESPH_CALDATA', async (req, res) => {
  //-------------------------------------
  console.log("--HESPH_CALDATA--");
  let input = req.body;
  let CALDATA = [];
  ...
  try {
    ...
    console.error("HESPH_CALDATA error:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
